...
static char *sec_to_hms(double seconds, char *buffer, size_t buffer_size)
{
    ...
    double fractional_seconds = fmod(seconds, 1.0);

    int hours = (int) (whole_seconds / 3600);
    int remaining_seconds = (int) whole_seconds % 3600;
    int minutes = remaining_seconds / 60;
    int secs = remaining_seconds % 60;

    int ret = snprintf(buffer, buffer_size, "%d:%02d:%02d.%03f",
                       hours, minutes, secs, fractional_seconds);
